        ...
        if (isAzure) {
            ...
            install("azure-interceptor") {
                plugin(HttpSend).intercept { request ->
                    ...
                    val url = toOpenAIHost(withAzureDeploymentId = path.last() != "models")
                        .toUrl()
        ...
        return ChatCompletionRequest(
            ...
            messages = listOf(
                ChatMessage(
                    ...
                    messageContent = TextContent(listOf(
...
fun OpenAISettings.toOpenAIHost(withAzureDeploymentId: Boolean): OpenAIHost = baseUrl.let { baseUrl ->
    if (baseUrl.isEmpty()) {
        OpenAIHost.OpenAI
    } else OpenAIHost(
        baseUrl = URLBuilder()
...
fun OpenAIHost.toUrl(): URLBuilder = URLBuilder()
    .takeFrom(baseUrl).also {
